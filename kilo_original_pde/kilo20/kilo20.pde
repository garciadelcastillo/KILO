...
class Calculation {
  ...
  void displayBeamDiagramAxial() {drawBeamDiagram(0); if (showValues==true) {drawBeamDiagramValues(0);}}
  void displayBeamDiagramShear() {drawBeamDiagram(1); if (showValues==true) {drawBeamDiagramValues(1);}}
  void displayBeamDiagramBending() {drawBeamDiagram(2); if (showValues==true) {drawBeamDiagramValues(2);}}
  void displayBeamDiagramDeformed() {drawBeamDiagram(3); if (showValues==true) {drawBeamDiagramValues(3);}}
...
color redDark=color(100, 0, 0); color redLight=color(200, 0, 0, 100); color redBright=color(255, 0, 0);
color greenDark=color(0, 100, 0); color greenLight=color(0, 200, 0, 100);
color blueDark=color(0, 0, 100); color blueLight=color(0, 0, 200, 100);
float EI, EIScale;
...
void drawLoadPuntual(float magnitude) {
  ...
  if (showValues==true) {
    ...
    if (magnitude>=0) {text(int(magnitude)+" kN", 0, -magnitude-2);}
    else {text(int(magnitude)+" kN", 0, -magnitude+10);}
...
void drawLoadMoment(float magnitude) {
  ...
  if (showValues==true) {
    ...
    if (magnitude>0) {text(int(magnitude)+" kN·m", 0, magnitude+18);}
    if (magnitude<0) {text(int(magnitude)+" kN·m", 0, -magnitude+18);}
    textFont(txt13); textAlign(LEFT); noFill();
  }
  strokeWeight(1);
...
void drawLoadLineal(float ri, float rf, float magnitude) {
  ...
  if (showValues==true) {
    textFont(txt10); textAlign(CENTER); fill(greenDark);
    if (magnitude>=0) {text(int(magnitude)+" kN/m", longitude/2, (rf-ri)*(b1.yf-b1.yi)/2-magnitude-2);}
    else {text(int(magnitude)+" kN/m", longitude/2, (rf-ri)*(b1.yf-b1.yi)/2-magnitude+10);}
    textFont(txt13); textAlign(LEFT); noFill();
  }
  strokeWeight(1);
}
...
void drawReactionArrows(int i) {
  stroke(greenDark); strokeWeight(1.5);
  pushMatrix(); 
...
void drawBeamDiagram(int kind) {  // Kind means: 0=Axial, 1=Shear, 2=Bending, 3=Deformed shape
  ...
  switch(kind){
    ...
    case 3: stroke(redBright); strokeWeight(2.5); dScale=EIScale; break;
  }  
  pushMatrix();
  rotate(b1.slope);
...
void drawBeamDiagramValues(int kind) {  // Kind means: 0=Axial, 1=Shear, 2=Bending, 3=Deformed shape
  ...
  if (showAllLoads==true) {
    for (int i=0; i<2; i++) {
      ...
      if (kind!=3) {if (int(b1.topValueTotal[i][kind])!=0) {text(int(b1.topValueTotal[i][kind])+units, 0, 0);}}
    ...
    for (int i=0; i<2; i++) {
      ...
      if (kind!=3) {if (int(b1.topValueLoad[currentLoad][i][kind])!=0) {text(int(b1.topValueLoad[currentLoad][i][kind])+units, 0, 0);}}
...
void keyPressed() {    
  switch(key) {
    ...
    case '1': case '2': case '3': case '4': case '5': 
      String temp=Character.toString(key); kbChangeActiveLoads(int(temp)); break;
    case 'Z': case 'z': welcomeToggle=true; break;
}//    case CODED:
      switch(keyCode) {
case F1: helpToggle=!helpToggle; break;
case SHIFT: welcomeToggle=false; break;
      }        
//  }
}
// Help text lines
...
void displayDataText() {
  ...
  switch(load[currentLoad].kindN){
    case 0: text("Magnitude = "+int(load[currentLoad].magnitudeY)+" kN", 10, 120); break;
    case 1: text("Magnitude = "+int(load[currentLoad].magnitude)+" kN·m", 10, 120); break;
    case 2: text("Magnitude = "+int(load[currentLoad].magnitudeY)+" kN/m (width "+nf((load[currentLoad].rf-load[currentLoad].ri)*b1.Lm, 0, 2)+" m)", 10, 120); break;
  }
