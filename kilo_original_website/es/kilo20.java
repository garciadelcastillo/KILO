...
public void keyboardInteraction() {
  if (keyPressed==true) {
    ...
    // Contraints of relative position values (can't use a if.else structure because on uniform)
    if (load[currentLoad].ri>1) {load[currentLoad].ri=1;} else if (load[currentLoad].ri<0) {load[currentLoad].ri=0;}
    if (load[currentLoad].rf>1) {load[currentLoad].rf=1;} else if (load[currentLoad].rf<0) {load[currentLoad].rf=0;}
