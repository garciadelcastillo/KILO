...
public class kilo20 extends PApplet {
...
// Declare sketch vars
Highlight highlight;
boolean helpToggle=false; boolean welcomeToggle=true;
...
//  txt16=createFont("Arial", 16); txt13=createFont("Arial", 13); txt10=createFont("Arial", 10);
  ...
  node[0]=new Node('R', 0.00f);  // created with relative position to the beam
  node[1]=new Node('S', 1.00f);  // id.
  load[0]=new Load('P', 0.50f, 0.75f, 100);
...
public void kbCursorModeRotationUp() {
  beamModeRotationAngleTemp=b1.slope-.01f; b1.xf=b1.L*cos(beamModeRotationAngleTemp); b1.yf=b1.L*sin(beamModeRotationAngleTemp);
}
...
public void kbChangeModeLoad(int toggle) {
  ...
  } else {if (cursorModeLoad==false) {cursorModeLoad=true;} else {currentLoad+=1; if (currentLoad==activeLoads) {currentLoad=0;}}
...
public void keyboardInteraction() {
  if (keyPressed==true) {
    switch(key) {
      ...
      case 'E': case 'e': kbChangeLoadWidth(-1); break; 
      case 'Y': case 'y': kbChangeEI(+1); break;
      case 'H': case 'h': kbChangeEI(-1); break;
      case CODED:
...
public void keyPressed() {    
  switch(key) {
    ...
    case 'S': case 's': kbChangeDisplayDiagram(1); break;
    case 'B': case 'b': kbChangeDisplayDiagram(2); break;
...
public void displayWelcomeScreen() {
  ...
  fill(redDark); text("INTERACTIVE STRUCTURAL ANALYSIS TOOL", 0, 20);
  ...
  textFont(txt13); fill(redDark); text("Press CAPS to continue", 0, 500);
  popMatrix();
...
public void displayDataText() {
  ...
  switch(load[currentLoad].kindN){
    case 0: text("Magnitude = "+PApplet.parseInt(load[currentLoad].magnitudeY)+" kN", 10, 120); break;
    case 1: text("Magnitude = "+PApplet.parseInt(load[currentLoad].magnitude)+" kN\u00b7m", 10, 120); break;
    case 2: text("Magnitude = "+PApplet.parseInt(load[currentLoad].magnitudeY)+" kN/m (width "+nf((load[currentLoad].rf-load[currentLoad].ri)*b1.Lm, 0, 2)+" m)", 10, 120); break;
  }
  ...
  fill(greenDark); text("AXIAL FORCES", 10, 190);
  if (showAllLoads==true) {
    text("Min. value = "+PApplet.parseInt(b1.topValueTotal[0][0])+" kN (at "+nf(b1.topIndexTotal[0][0]*0.01f, 0, 2)+" m)", 10, 200);
    text("Max. value = "+PApplet.parseInt(b1.topValueTotal[1][0])+" kN (at "+nf(b1.topIndexTotal[1][0]*0.01f, 0, 2)+" m)", 10, 210);
  } else {
    text("Min. value = "+PApplet.parseInt(b1.topValueLoad[currentLoad][0][0])+" kN (at "+nf(b1.topIndexLoad[currentLoad][0][0]*0.01f, 0, 2)+" m)", 10, 200);
    text("Max. value = "+PApplet.parseInt(b1.topValueLoad[currentLoad][1][0])+" kN (at "+nf(b1.topIndexLoad[currentLoad][1][0]*0.01f, 0, 2)+" m)", 10, 210);
  }
  fill(blueDark); text("SHEAR FORCES", 10, 230);
  if (showAllLoads==true) {
    text("Min. value = "+PApplet.parseInt(b1.topValueTotal[0][1])+" kN (at "+nf(b1.topIndexTotal[0][1]*0.01f, 0, 2)+" m)", 10, 240);
    text("Max. value = "+PApplet.parseInt(b1.topValueTotal[1][1])+" kN (at "+nf(b1.topIndexTotal[1][1]*0.01f, 0, 2)+" m)", 10, 250);
  } else {
    text("Min. value = "+PApplet.parseInt(b1.topValueLoad[currentLoad][0][1])+" kN (at "+nf(b1.topIndexLoad[currentLoad][0][1]*0.01f, 0, 2)+" m)", 10, 240);
    text("Max. value = "+PApplet.parseInt(b1.topValueLoad[currentLoad][1][1])+" kN (at "+nf(b1.topIndexLoad[currentLoad][1][1]*0.01f, 0, 2)+" m)", 10, 250);
  }
  fill(redDark); text("BENDING FORCES", 10, 270);
  if (showAllLoads==true) {
    text("Min. value = "+PApplet.parseInt(b1.topValueTotal[0][2])+" kN\u00b7m (at "+nf(b1.topIndexTotal[0][2]*0.01f, 0, 2)+" m)", 10, 280);
    text("Max. value = "+PApplet.parseInt(b1.topValueTotal[1][2])+" kN\u00b7m (at "+nf(b1.topIndexTotal[1][2]*0.01f, 0, 2)+" m)", 10, 290);
  } else {
    text("Min. value = "+PApplet.parseInt(b1.topValueLoad[currentLoad][0][2])+" kN\u00b7m (at "+nf(b1.topIndexLoad[currentLoad][0][2]*0.01f, 0, 2)+" m)", 10, 280);
    text("Max. value = "+PApplet.parseInt(b1.topValueLoad[currentLoad][1][2])+" kN\u00b7m (at "+nf(b1.topIndexLoad[currentLoad][1][2]*0.01f, 0, 2)+" m)", 10, 290);
  }
  fill(redBright); text("DEFORMED SHAPE", 10, 310);
  if (showAllLoads==true) {
    text("Min. value = "+nf(1000*b1.topValueTotal[0][3], 0, 2)+" mm (at "+nf(b1.topIndexTotal[0][3]*0.01f, 0, 2)+" m)", 10, 320);
    text("Max. value = "+nf(1000*b1.topValueTotal[1][3], 0, 2)+" mm (at "+nf(b1.topIndexTotal[1][3]*0.01f, 0, 2)+" m)", 10, 330);
  } else {
    text("Min. value = "+nf(1000*b1.topValueLoad[currentLoad][0][3], 0, 2)+" mm (at "+nf(b1.topIndexLoad[currentLoad][0][3]*0.01f, 0, 2)+" m)", 10, 320);
    text("Max. value = "+nf(1000*b1.topValueLoad[currentLoad][1][3], 0, 2)+" mm (at "+nf(b1.topIndexLoad[currentLoad][1][3]*0.01f, 0, 2)+" m)", 10, 330);
  }
  noFill(); textFont(txt13);
}
...
public void eraseAllCalculation() {
  ...
  static public void main(String args[]) {
    PApplet.main(new String[] { "--present", "--bgcolor=#666666", "--stop-color=#cccccc", "kilo20" });
  }
}
